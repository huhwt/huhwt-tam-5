...
var PARAM_RENDER_UPDATE_INTERVAL = 2;        // number of simulation updates before updating the tick visualization
var PARAM_SHADING = true;
var PARAM_COLORMAP = d3.interpolateGnBu;
var PARAM_REVERSE_COLORMAP = false;
var PARAM_USE_MOUSEOVER = false;
var PARAM_SHOW_YEARVALUES = false;
var PARAM_SHOW_GRAPH = true;
var PARAM_SHOW_CONTOURS = false;
...
class TAMRenderer
{
    ...
    createForceGraphJSON(json)
    {
        ...
        Object.values(json.links).forEach(link => {
            ...
            if (source && target)
            {
                let _link = {"source": source, "target": target, "directed": link.directed, "distance": PARAM_LINK_DISTANCE };
    ...
    placeLabel(node)
    {
        if (PARAM_PERSON_LABELS_BELOW_NODE)
        {
            ...
            let y = node.y + node.r + 1.0 * PARAM_FONT_SIZE;
        ...
        else
        {
            ...
            let y = node.y + PARAM_FONT_SIZE/4;
            return "translate(" + x + ", " + y + ")";
        }
    }
    ...
    createTunnels(SCALARFIELD, SEGMENTS)
    {
        ...
        SEGMENTS.forEach(segment => 
        {
            ...
            let offset = Math.min( d / 2, segment.r1 + PARAM_ARROW_DISTANCE_FACTOR * PARAM_ARROW_RADIUS);
            v = v.mul(offset/d);
            ...
            if (!v.zero())
            {
                ...
                let wasUnderground = SCALARFIELD.sampleBilinear(pv0.x, pv0.y) - pv0.z > PARAM_UNDERGROUND_THRESHOLD;
                ...
                for (let i = 0, pv = pv0; i < nsteps; i++, pv = pv.add(v))
                {
                    ...
                    let isUnderground = sfValue - pv.z > PARAM_UNDERGROUND_THRESHOLD;
