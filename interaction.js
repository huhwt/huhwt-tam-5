...
function setTAMInteractions()
{
    ...
    d3.select("body")
        .on("keydown", function() {
            ...
            else if (d3.event.keyCode == i18n("kc_Y").charCodeAt(0)) {
                toggleShowYearValues();
                d3.select("#settings_show_yearvalues").property('checked', PARAM_SHOW_YEARVALUES);
            }
            ...
            else if (d3.event.keyCode == i18n("kc_M").charCodeAt(0)) {
                toggleShowContours();
...
function toggleShowContours()
{
    ...
    renderer.TOPO_LAYER.attr("visibility", PARAM_SHOW_CONTOURS || PARAM_SHOW_YEARVALUES ? "visible" : "hidden");
    renderer.SHADING_LAYER.attr("visibility", PARAM_SHOW_CONTOURS && PARAM_SHADING ? "visible" : "hidden");
}
//---------------------------------------------------------------------------
function toggleShowYearValues()
{
    PARAM_SHOW_YEARVALUES = !PARAM_SHOW_YEARVALUES;
...
function toggleSelectTime()
{
    ...
    if (PARAM_USE_MOUSEOVER || PARAM_SHOW_YEARVALUES) {
...
{
    let active_language = i18n("ZZZZ");
    if (active_language != PARAM_ACTIVE_LOCALE) {
        switch_locale(PARAM_ACTIVE_LOCALE);
...
function registertooltipYVeventhandler()
{
    if (PARAM_SHOW_YEARVALUES) {
        let tooltip = d3.select("#tooltipYV");
        tooltip.style("visibility", "visible");
        if (renderer.SVG_CONTOURS) {
            renderer.SVG_CONTOURS
                .on("mouseover", function (c) {
                    // return tooltip.style("visibility", "visible");
                })
                .on("mouseenter", function (c) { // insert tooltip content
